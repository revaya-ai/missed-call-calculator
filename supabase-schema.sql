-- Missed Call Calculator Submissions Table
...
CREATE TABLE calculator_submissions (
  ...
  average_calls_per_day INTEGER NOT NULL CHECK (average_calls_per_day >= 0),
  estimated_missed_call_percentage DECIMAL(5,2) NOT NULL CHECK (estimated_missed_call_percentage >= 0 AND estimated_missed_call_percentage <= 100),
  average_job_value DECIMAL(10,2) NOT NULL CHECK (average_job_value >= 0),
  close_rate_percentage DECIMAL(5,2) NOT NULL CHECK (close_rate_percentage >= 0 AND close_rate_percentage <= 100),
